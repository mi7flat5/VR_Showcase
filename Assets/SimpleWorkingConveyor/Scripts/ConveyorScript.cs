...
public class ConveyorScript : MonoBehaviour {
    ...
    public float GetConveyorSpeed() { return conveyorDriveSpeed; }
  public  void SetSpeed() {

        if (driveIndex == 2)
        {
            conveyorDriveSpeed = 1.0f;
            ++driveIndex;
            SendMessageUpwards("UpdateConveyorSpeeds");
            return;
        }
        if (driveIndex == 3)
        {
            conveyorDriveSpeed = 3.0f;
            driveIndex = 0;
            SendMessageUpwards("UpdateConveyorSpeeds");
            dinger.clip = ding;
            dinger.Play();
