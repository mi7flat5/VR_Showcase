...
public void ManageIndicator()
    {
        if (!teleport)
        {
            ...
            if (Vector3.Distance(raycastIndicator.transform.position, player.transform.position) <= 3.0F)
            {
                moving = false;
                if (moving == false)
                {
                    raycastIndicator.transform.position = player.transform.position;
                    raycastIndicator.transform.position +=  3.0F*player.transform.forward;
                }
            }
